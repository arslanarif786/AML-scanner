...
<q-spinner-dots
          ...
          class="save-loader"
/>
  <q-splitter v-model="splitterModel" style="height: auto; padding: 20px" :class="saveLoader ? 'disabledContent' : ''">
      ...
      <q-splitter v-model="contentSplitterModel" class="q-pt-lg q-mt-lg">
        <template v-slot:before>
          <!--
               ...
               -->
          <UploadReportImage :thumbnail="reportThumbnail" @image-uploaded="imageUploaded" />
  ...
  >
    ...
    >
      ...
      <div class="row justify-center items-center" v-if="loader">
      ...
      <ReportSectionFields
        ...
        @save-report="updateReport(report, route.params['id'])"
      />
    </q-tab-panel>
  </q-tab-panels>
</template>
...
export default {
  ...
  methods: {
      ...
      if (this.validatSactionFields()) {
        ...
        this.apiRequest(
          ...
          .then(() => {
            ...
            this.success("Report has been Updated successfully");
          })
          .finally(() => {
            this.saveLoader = false;
          });
    ...
    updateReportOld(report, id) {
      this.loading = true;
      let array = Object.entries(report);
  ...
  mounted() {
    ...
    this.apiRequest(`search/reports/${id}`, "report", "GET", {}).then((res) => {
      this.report = res.data.data;
      this.tab = this.report.report_sections[0].id;
      this.reportThumbnail = this.report.report_thumbnail;
    ...
    EventBus.on("image-uploaded", (data) => {
      ...
      console.log(this.reportThumbnail, "This is report Thumbnail");
    });
