...
<script>
/**
 * Description : This componnet is use to filter data
 * @props {String} label
 * @props {String} icon
 * @props {Boolean} allowSearch
 * @props {allowDateRange} allowSearch
 * @props {allowDateRange} allowSearch
 * @props {Array} Options
 * @props {Boolean} isCountry
 * @props {Boolean} isDateRange
 * @props {Boolean} isDateRange
 * @props {String} optionValue
 * @props {String} optionKey
 */
...
export default {
  ...
  methods: {
    ...
    selectOption(option,type) {
      ...
      type ? type=='date' ? option.to ? option.to.length > 2 ? this.$emit("filter-value", option) : this.error('Please select date range correctly') :this.error('Please select date range correctly'):'':this.$emit("filter-value", option);
