<template>
  <Modal :show="open" @closeModal="closeModal" title="Create Role">
    <template v-slot:body>
      <div class="">
        ...
        >
          ...
          <label v-if="item.field == 'text'" for="">{{ item.label }}</label>
          <q-input
          ...
          <label v-if="item.field == 'dropdown'" class="q-mt-md">{{
            item.label
          }}</label>
          <q-select
            class="q-mt-md"
...
<script>
/**
 * Description : This component is used to create new role 
 * @props {Boolean } show
 * @props {Array } emittedPermissions
 */
...
export default {
  ...
  methods: {
    ...
    createRole() {
      this.loading = true;
      this.apiRequest("agent/role/store", "agent", "POST", {
