        ...
        <div class="col q-pl-md q-pt-sm">
          <div class="text-h6 font-16px q-px-sm cursor-pointer">
            ...
            >
              {{
                processReportName(
                  url.source[0] ? url.source[0].name : url.origin
                )
              }}
            </a>
          </div>
          ...
          >
            Countries:
            <div
              v-if="country.short_code"
              :class="`flag flag-${country ? country.short_code.toLowerCase() : 'No Country'} q-ml-sm`"
            >
              <q-tooltip>{{
                url.source[0] ? url.source[0].country : "No Country"
              }}</q-tooltip>
            </div>
          </div>
        </div>
    ...
    <q-card-actions>
      ...
      >
        ...
        <q-chip
          ...
          v-if="url.source[0] && url.source[0].category"
          >{{ url.source[0].category }}</q-chip
        >
...
export default {
  ...
  mounted() {
    ...
    this.country = this.reportCountry.find(
      (e) => e.name === this.url.source[0].country
    );
  },
  computed: {
    createdAt() {
