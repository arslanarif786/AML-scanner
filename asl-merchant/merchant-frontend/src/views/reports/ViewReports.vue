<template>
  <div class="q-pa-md">
    ...
    <div v-else class="q-pa-md q-ma-none row items-start q-gutter-md">
      <template v-for="item in reports" :key="item.report_name">
    ...
    />
    <div
      class="d-flex justify-between align-center q-my-sm"
      v-if="totalReports > perPage"
    >
      <span class="color-secondary font-11px text-weight-medium"
        >Showing results
        <strong class="text-dark">{{
          currentPage * perPage > totalReports
            ? totalReports - (totalReports - perPage)
            : currentPage * perPage
        }}</strong>
        from <strong class="text-dark">{{ totalReports }}</strong></span
      >
      <q-pagination
        v-model="currentPage"
        :max="lastPage"
...
export default {
  ...
  methods: {
    ...
    handleRequest(request, country, from, to, name) {
      ...
      this.apiRequest(
        `search/reports?page=${request.page}&per_page=${request.rowsPerPage}&countries[0]=${country}&start_date=${from}&end_date=${to}&name=${name}`,
        "report",
        "GET",
        {}
      )
        .then((res) => {
          ...
          this.totalReports = res.data.data.total;
