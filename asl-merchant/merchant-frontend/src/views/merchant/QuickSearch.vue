<template>
  <section class="screen-bg x-padding default-height q-pt-sm">
    <div class="row justify-between q-pa-sm q-mt-md">
      ...
      <div>
        <span
          class="aml-search-heading text-weight-bold font-16px global-font-color"
        >
    ...
    <div class="screen-bg q-mt-sm q-pb-sm" style="margin: 0 10px 0 10px">
      ...
      />
      <div v-else class="q-pb-md q-ma-sm row items-start q-gutter-md">
        <template
      ...
      <div
        class="screen-bg d-flex justify-between align-center q-mr-lg q-ml-sm"
      >
...
export default {
  ...
  methods: {
     ...
     */
    getApiToken(secrets) {
      this.apiRequest("get-access-token", "report", "POST", secrets).then(
        (res) => {
    ...
    getReports(request) {
      this.loading = true;
      this.apiRequest(`search/reports?page=${request ? request.page : 1}`, "report", "GET")
        .then((res) => {
          this.pagination["page"] = res.data.data.current_page;
          this.pagination["perPage"] = res.data.data.per_page;
          this.pagination["lastPage"] = res.data.data.last_page;
  ...
  mounted() {
    this.getApiToken(JSON.parse(localStorage.getItem("secrets")));
    this.getReports();
    ...
    EventBus.on("pagination-request", (pageNo) => {
      if (this.name) {
        ...
        this.getReports({ page: pageNo, perPage: "10" });
      }
    });
